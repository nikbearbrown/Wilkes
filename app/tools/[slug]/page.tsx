...
export async function generateMetadata({
  ...
  if (doc) {
    return {
      title: `${doc.title} - Wilkes Tools`,
      description: doc.description || `${doc.title} — tool by Wilkes`,
    }
  }
  ...
  try {
    ...
    if (rows.length > 0) {
      return {
        title: `${rows[0].name} - Wilkes Tools`,
        description: rows[0].description || `${rows[0].name} — tool by Wilkes`,
      }
    }
  } catch {}

  return { title: 'Tool - Wilkes' }
}
